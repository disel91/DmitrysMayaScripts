...
class GearUI(BaseWindow):
    ...
    def buildUI(self):
        ...
        self.slider = cmds.intSlider(min=5, max=30, value=5, step=1, dragCommand=self.modifyGear)
        cmds.button(label="Make gear", command=self.makeGear)
        cmds.button(label="Bevel", command=self.bevelGear)
        cmds.button(label="Reset", command=self.reset)
